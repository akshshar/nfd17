...
CONFIG_FILES_MAP = {"FOC2137R1MX" : "rtr1.config", 
                    "FOC2128R08L" : "rtr2.config", 
                    "FGE194714QS" : "rtr3.config"}

OPENR_CONFIG_FILES_MAP = {"FOC2137R1MX" : "run_openr_rtr1.sh",
                         "FOC2128R08L" : "run_openr_rtr2.sh",
                         "FGE194714QS" : "run_openr_rtr3.sh"}

OPENR_HOSTS_FILES_MAP = {"FOC2137R1MX" : "hosts_rtr1",
                         "FOC2128R08L" : "hosts_rtr2",
                         "FGE194714QS" : "hosts_rtr3"}


K9SEC_PACKAGE = "ncs5500-k9sec-3.2.0.0-r622.x86_64.rpm"
MGBL_PACKAGE = "ncs5500-mgbl-3.0.0.0-r622.x86_64.rpm"
SYSLOG_SERVER = "11.11.11.2"
SYSLOG_PORT = 514
SYSLOG_LOCAL_FILE = "/root/ztp_python.log"
CRON_SCRIPT = "periodic.py"
...
if __name__ == "__main__":
    ...
    docker_sysconfig_file_url = SERVER_URL+"docker"

    output = ztp_script.download_file(docker_sysconfig_file_url, destination_folder="/misc/app_host/scratch")

    if output["status"] == "error":
        if ztp_script.debug:
            ztp_script.logger.debug("File Download failed, aborting script")
            ztp_script.syslogger.info("File Download failed, aborting script")

            sys.exit(1)            

    elif output["status"] == "success":
        if ztp_script.debug:
            ztp_script.logger.debug("File Download complete, copying to /etc/sysconfig")

            ztp_script.syslogger.info("File Download complete, copying to /etc/sysconfig")

        file_name = output["filename"]
        file_location = output["folder"]
        file_path = os.path.join(file_location, file_name)

        result = ztp_script.run_bash("cp "+file_path+" /etc/sysconfig/docker")
        if (result["status"]):
            if ztp_script.debug:
                ztp_script.logger.debug("Failed to copy downloaded file to /etc/sysconfig")
                ztp_script.syslogger.info("Failed to copy downloaded file to /etc/sysconfig")
        else:
            time.sleep(120)
        ...
        try:
            ...
            result["warning"] = "failed to remove file from path: "+str(file_path)

        result = ztp_script.run_bash("DOCKER_HOST=\"unix:///misc/app_host/docker.sock\" docker pull 11.11.11.2:5000/openr")
        if(result["status"]):
            if ztp_script.debug:
                ztp_script.logger.debug("Failed to pull docker image")
                ztp_script.syslogger.info("Failed to pull docker image")
                ztp_script.logger.debug(result["output"])
                sys.exit(1)            

        result = ztp_script.run_bash("DOCKER_HOST=\"unix:///misc/app_host/docker.sock\" docker images | grep -q openr")
        if(result["status"]):
            if ztp_script.debug:
                ztp_script.logger.debug("Failed to execute docker images command")
                ztp_script.syslogger.info("Failed to execute docker images command")
                ztp_script.logger.debug(result["output"])
                sys.exit(1) 
    ...
    download_cron = ztp_script.download_file(SERVER_URL_SCRIPTS + CRON_SCRIPT, destination_folder="/misc/app_host")
    ...
    # Finally allow direct access to shell be enabling sshd_operns

    result = ztp_script.run_bash("service sshd_operns restart")
    if(result["status"]):
        if ztp_script.debug:
            ztp_script.logger.debug("Failed to restart sshd_operns")
            ztp_script.syslogger.info("Failed to restart sshd_operns")
            ztp_script.logger.debug(result["output"])
            sys.exit(1)


    # Dowload the files for openr (hosts file and configuration file)
    OPENR_CONFIG_FILE= OPENR_CONFIG_FILES_MAP[serial_number["output"].rstrip()]
    OPENR_HOSTS_FILE= OPENR_HOSTS_FILES_MAP[serial_number["output"].rstrip()]

    # Download Config with Mgmt vrfs
    output = ztp_script.download_file(SERVER_URL_CONFIGS + OPENR_CONFIG_FILE, destination_folder="/misc/app_host")
    ...
    #Make sure the downloaded openr config file/script is executable


    result = ztp_script.run_bash("chmod 777 "+file_path)
    if(result["status"]):
        if ztp_script.debug:
            ztp_script.logger.debug("Failed to pull docker image")
            ztp_script.syslogger.info("Failed to pull docker image")
    ...
    # Download Config with Mgmt vrfs
    output = ztp_script.download_file(SERVER_URL_CONFIGS + OPENR_HOSTS_FILE, destination_folder="/misc/app_host")
